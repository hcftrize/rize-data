...
TIMEOUT       = 30
WINDOW_DAYS   = 7   # days to refresh on normal run
...
def fetch_day(d: date) -> float | None:
    """Fetch volume for a specific date from CoinGecko /history endpoint."""
...
def main():
    ...
    # Build lookup from existing JSON
    by_date = {p["date"]: p["volume"] for p in existing_series}

    window = 365 if bootstrap else WINDOW_DAYS
    print(f"=== {'BOOTSTRAP' if bootstrap else 'INCREMENTAL'} — fetching last {window} days from /history ===")
